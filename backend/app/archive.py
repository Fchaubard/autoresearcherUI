...
from . import metrics
from .config import ARTIFACTS_DIR, DATA_DIR, DB_PATH, ROOT
...
def _walk_files():
    """Yield (relative_path, size) for every file under DATA_DIR, skipping the
    archive's own scratch dirs."""
        ...
        with open(listfile, "w") as lf:
            for rel, sz in _walk_files():
                ...
                if "__pycache__" in rel or sz >= _SLIM_CAP:
                    continue
                lf.write(rel + "\n")
        cmd += ["-C", str(DATA_DIR), "--files-from", str(listfile)]
    else:
        members = [d for d in ("workspace", "artifacts")
                   if (DATA_DIR / d).exists()]
        cmd += ["-C", str(DATA_DIR)] + members
...
def restore(tar_path: str) -> dict:
    ...
    # restore the workspace + artifacts trees
    for d in ("workspace", "artifacts"):
        src = _RESTORE / d
        if src.exists():
            dest = DATA_DIR / d
            if dest.exists():
